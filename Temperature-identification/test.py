...
def handle(img,f,x):
    ...
    num = pytesseract.image_to_string(img_L1, lang='num', config='-psm 6 -c tessedit_char_whitelist="-0123456789"')
    #负号检测
    if "-" in num:
        num= minus(img_L1,f)+ num[1:]
    return num

#负号检测
def minus(img,f):
    img_minus=img.crop([0,img.size[1]/3,img.size[0]*2/3,img.size[1]*2/3])
    widenoise(img_minus)
    #img_minus.save(f+"_minus.jpg")
    pixdata = img_minus.load()
    w,h = img_minus.size
    count = 0
...
def widenoise(img):
    for i in range(img.width):
        if img.getpixel((i, 0)) == 1:
            expand(img, i, 0)
        if img.getpixel((i, img.height - 1)) == 1:
            expand(img, i, img.height - 1)
    for j in range(img.height):
        if img.getpixel((0, j)) == 1:
            expand(img, 0, j)
        if img.getpixel((img.width - 1, j)) == 1:
            expand(img, img.width - 1, j)
def examine(img, x, y):
    width = img.width
    height = img.height
